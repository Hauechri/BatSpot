...
from animal_spot.data import transforms as T
...
from animal_spot.utils.FileIO import AsyncFileReader
...
class Dataset(AudioDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if augmentation:
            self._logger.debug("Init augmentation transforms for time shift and amplitude augmentation.")
            self.t_amplitude = T.RandomAmplitude(3, -6)
            self.t_timestretch = T.RandomTimeStretch()
            # self.t_pitchshift = T.RandomPitchSift() #SQS
        else:
        ...
        if augmentation:
            if noise_files:
                ...
                self.t_addnoise = T.RandomAddNoise(
                    noise_files,
                    self.t_spectrogram,
                    T.Compose(self.t_timestretch, self.t_compr_f), #SQS self.t_pitchshift,
                    min_length=seq_len,
    ...
    def __getitem__(self, idx):
        ...
        if self.augmentation:
            ...
            # sample = self.t_pitchshift(sample) #SQS
